...
function DiagramNode({
  ...
  return (
    <div className="relative rounded-2xl border border-purple-500/15 bg-gradient-to-br from-purple-500/[0.06] to-transparent px-4 py-4 transition-all hover:scale-[1.01] hover:border-purple-500/25">
      {number !== undefined && (
        <div className="absolute -top-2 left-3 rounded-full border border-purple-500/25 bg-gradient-to-r from-purple-600/20 to-purple-800/20 px-2 py-0.5 text-[10px] font-bold text-purple-300">
          {number}
        </div>
      )}
      <div className="flex items-start gap-3">
        ...
        <div>
          ...
          <p className="mt-0.5 text-xs leading-relaxed text-[hsl(var(--muted-foreground))]">{desc}</p>
        </div>
      </div>
    </div>
  )
...
export function DisputeFlowDiagram() {
  return (
    <div className="flex flex-col gap-0">
      <DiagramNode
        ...
        desc="Client or freelancer explains the problem on-chain. The milestone is frozen &mdash; no money moves until this is resolved."
      />
      ...
      <DiagramNode
        ...
        desc="The smart contract triggers NEAR AI Cloud. The AI analyzes the contract details, milestone info, and chat history in multiple rounds &mdash; each round goes deeper. It checks claims, cross-references the chat, and evaluates evidence."
      />

      <VerticalLine />

      {/* TEE detail */}
      <div className="rounded-2xl border border-purple-400/15 bg-gradient-to-br from-purple-500/[0.05] to-violet-600/[0.03] p-4">
        ...
        <div className="grid gap-2 sm:grid-cols-3">
          {[
            ...
            { step: "Contract verifies", desc: "Smart contract calls ed25519_verify &mdash; creates a full audit trail" },
          ].map((item, i) => (
            <div key={i} className="flex items-start gap-2 rounded-xl border border-purple-500/10 bg-[hsl(240_8%_7%/0.5)] px-3 py-2.5">
              ...
              <div>
                <p className="text-[11px] font-medium text-[hsl(var(--foreground))]">{item.step}</p>
                <p className="text-[10px] leading-relaxed text-[hsl(var(--muted-foreground))]">{item.desc}</p>
              </div>
            </div>
          ))}
        </div>
      ...
      <DiagramNode
        ...
        desc="The final round produces a resolution: pay the freelancer, refund the client, or split the funds. The AI explains its reasoning &mdash; visible to everyone."
      />

      <VerticalLine />

      {/* Accept vs Appeal */}
      <div className="rounded-2xl border border-purple-500/15 bg-[hsl(240_8%_7%/0.5)] p-4">
        <p className="mb-3 text-center text-xs font-semibold uppercase tracking-widest text-[hsl(var(--muted-foreground))]">
          Both parties choose
        </p>
